...
| `--assets sidecar` | write attachments to a sibling `.khba` pack instead of embedding them in the `.khb` — see [Images & assets](images.md) |

## What the compiler validates

A broken book fails the compile instead of shipping broken:

- every `page:` id in [toc.yaml](toc-yaml.md) must name an existing page, and a folder
  node must have a `title:`;
- every in-book id in a page's [`related`](frontmatter-related.md) list must exist
  (cross-book `docsetId:pageId` entries are stored as-is — the other book compiles
  separately);
- every [math](math.md) formula must parse — the error names the page and the offending
  LaTeX;
- the [code](code-blocks.md) containers must be well-formed: a
  `~~~code-group` or `~~~code-tree` with no inner blocks, or a `~~~code-preview`
  missing its command or output block, is a build error;
- the YAML [frontmatter](frontmatter.md) block must parse (and be terminated).
