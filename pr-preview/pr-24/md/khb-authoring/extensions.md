...
images the program produced as [assets](images.md).
...
Add an `[extensions.<name>]` table to [`docset.toml`](docset-toml.md). The `<name>` is what the
...
[Diagrams](diagrams.md)). Compiling *without* the flag is always safe: each `ext:` block is left
...
[diagrams](diagrams.md) and [math](math.md).
