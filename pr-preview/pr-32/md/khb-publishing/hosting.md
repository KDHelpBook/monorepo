...
GitHub Pages **honours HTTP `Range`**, so [streamed](pack-stream.md) books work
...
file. See [Streaming](pack-stream.md) for the packing side.

## Compressed transfer (`.khb.gz`)

Files packed with [`--mode compact`](pack-mode.md) ship pre-gzipped as
...
e.g. *File → Open docset from URL…* or a [.khbm manifest](khbm-manifests.md)
...
> site** ([versioning](versioning.md)) instead of linking to release assets: releases
