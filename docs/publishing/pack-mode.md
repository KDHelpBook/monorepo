...
keywords: [mode, compact, gzip, khb.gz, compression, transfer size]
categories: [packing]
related: [pack, pack-stream, hosting]
---
...
> [!NOTE]
> **Exceptions to `compact`:** docsets marked for [streaming](pack-stream) ship
> uncompressed even in compact mode (`Range` addresses raw bytes), and the
> [`--llms`](pack-llms) export stays plain text (it's meant to be read as-is).
