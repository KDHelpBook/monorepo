---
title: TOC ordering
keywords: [ordering, order, file names, numeric prefixes, flat toc, fallback]
categories: [configuration]
related: [toc-yaml, toc-nodes, frontmatter-id]
---
...
If the book has no [toc.yaml](toc-yaml), the compiler produces a **flat** table of
...
> page sets an explicit [`id`](frontmatter-id) in its frontmatter. If the book might
...
subpages or label overrides, add a `toc.yaml` — the [nodes](toc-nodes) page covers
the syntax.
